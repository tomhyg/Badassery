"""
Script d'enrichissement OPTIMISÉ v2 - Simple et Robuste
"""
...
RSS_CONCURRENT = 50     # Threads pour RSS
...
def parse_rss(rss_url):
    ...
    try:
        resp = requests.get(rss_url, timeout=10, headers={"User-Agent": "PodcastEnricher/1.0"})
...
def enrich_with_rss(podcasts):
    ...
    with ThreadPoolExecutor(max_workers=RSS_CONCURRENT) as ex:
        futures = [ex.submit(process, i, p) for i, p in enumerate(podcasts)]
...
def save_database(podcasts, columns, apple_data, rss_data):
    ...
    rss_cols = list(rss_data[0].keys()) if rss_data and rss_data[0] else []
    all_cols += rss_cols
...
def main():
    ...
    log("=" * 60)
    log("DÉMARRAGE - VERSION 2")
    log(f"Mode: {'TEST' if TEST_MODE else 'PRODUCTION'}")
    log("=" * 60)
