"""
TEST COMPLET V2 - 50 podcasts - TOUTES LES COLONNES
====================================================
RSS (+ 10 épisodes) + Apple Scraping + Website Scraping + YouTube (+ 10 vidéos)
"""
...
OUTPUT_FILE = r"C:\Users\admin\OneDrive\Bureau\Dossier danielle\test_50_results_v2.json"
TEST_LIMIT = 50
...
def enrich_website(podcasts):
    ...
    with_li = sum(1 for p in podcasts if p.get('website_linkedin'))
    ...
    log(f"Avec LinkedIn: {with_li}/{len(podcasts)}")
    log(f"Avec email (website): {with_email}/{len(podcasts)}")

    return podcasts

# ============== 5. YOUTUBE (yt-dlp + scraping) ==============
...
def enrich_youtube(podcasts):
    ...
    for idx, (i, p) in enumerate(with_yt):
        ...
        p.update(yt_data)
        log(f"YouTube: {idx + 1}/{len(with_yt)} - {p.get('yt_channel_name', 'N/A')} ({p.get('yt_subscribers', 'N/A')} subs, {p.get('yt_video_count', 'N/A')} videos)")
        time.sleep(1)
...
def print_final_stats(podcasts):
    log("=" * 60)
    log("STATISTIQUES FINALES - TOUTES LES COLONNES")
    log("=" * 60)
    ...
    categories = {
        'Base': [k for k in all_keys if k in ['id', 'rss_url', 'title', 'itunes_id', 'website', 'description', 'language']],
        'RSS Info': [k for k in all_keys if k.startswith('rss_') and '_ep' not in k],
        'RSS Épisodes': [k for k in all_keys if 'rss_ep' in k],
        'Apple': [k for k in all_keys if k.startswith('apple_')],
    ...
    important_cols = [
        'rss_owner_email', 'rss_ep1_title', 'apple_rating',
        'website_youtube', 'website_twitter', 'website_linkedin',
        'yt_channel_id', 'yt_subscribers', 'yt_vid1_title', 'yt_vid1_views', 'yt_vid1_likes'
    ]
...
def main():
    ...
    log("=" * 60)
    log("TEST COMPLET V2 - 50 PODCASTS - TOUTES COLONNES")
    log("=" * 60)
