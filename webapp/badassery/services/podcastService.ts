...
export const getAllPodcastsCached = async (): Promise<PodcastDocument[]> => {
  ...
  if (cachedPodcasts && (now - cacheTimestamp) < CACHE_TTL) {
    console.log('[PodcastService] Returning cached podcasts:', cachedPodcasts.length);
    return cachedPodcasts;
  }

  console.log('[PodcastService] Loading all podcasts from Firestore...');

  try {
    // Fetch ALL podcasts with badassery score (most useful for filtering)
    const q = query(
      collection(db, 'podcasts'),
      where('ai_badassery_score', '>', 0),
      orderBy('ai_badassery_score', 'desc'),
      limit(10000) // High limit to get all scored podcasts
    );
    ...
    cacheTimestamp = now;
    console.log('[PodcastService] Cached', cachedPodcasts.length, 'podcasts');
...
export const getPodcastStats = async (): Promise<{
  ...
  try {
    ...
    return {
      ...
      guestFriendly: allPodcasts.filter(p => p.ai_guest_friendly === true).length,
    ...
    return { total: 0, withEmail: 0, withYouTube: 0, highRated: 0, categorized: 0, guestFriendly: 0, highBusinessRelevance: 0 };
  }
};
  ...
  try {
    ...
    return allPodcasts
      .filter(p => {
        ...
        if (!p.ai_guest_friendly) return false;
...
export const getFilteredPodcasts = async (filters: {
  ...
  try {
    ...
    if (filters.guestFriendly !== undefined) {
      filtered = filtered.filter(p => p.ai_guest_friendly === filters.guestFriendly);
    }
...
export const getPodcastEmails = (podcast: PodcastDocument): PodcastEmailCandidate[] => {
  ...
  // Sort: primary first, then booking, then general
  results.sort((a, b) => {
    const typeOrder = { primary: 0, booking: 1, general: 2 };
...
export const getPodcastSocialLinks = (podcast: PodcastDocument): PodcastSocialLinks => {
  ...
  // YouTube link
  if (podcast.yt_channel_id) {
    links.youtube = `https://youtube.com/channel/${podcast.yt_channel_id}`;
  }

  // Website
  if (podcast.website || podcast.rss_website) {
    links.website = podcast.website || podcast.rss_website;
  }

  // Note: Twitter/Instagram would need to be scraped or added to the data model
